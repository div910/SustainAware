...
class Dao:
    ...
    def __init__(self):
        ...
        self.user_activity_table_name = 'user_activity'
        self.sustainable_activity_table_name = 'sustainable_activity'

    def get_posts_by_username(self, username):
        try:
            connection_obj = get_connection(self.database_config)
            query_params = {}
            query = f'select id, product_description_raw, product_description_frontend, image_name from {self.user_activity_table_name} where username = "{username}"'
    ...
    def add_posts_by_username(self, username, post):
        try:
            connection_obj = get_connection(self.database_config)
            query_params = {}
            query = f'select id, product_description_raw, product_description_frontend, image_name from {self.user_activity_table_name} where username = "{username}"'
